...
export function orderStats() {
  ...
  const checkedInPaid = db.prepare(`SELECT COUNT(*) AS n FROM order_guests g JOIN orders o ON o.id = g.order_id WHERE o.status = 'approved' AND g.removed_at IS NULL AND g.checked_in_at IS NOT NULL`).get() as Record<string, unknown>;
  const checkedInComplimentary = db.prepare(`SELECT COUNT(*) AS n FROM complimentary_guests WHERE removed_at IS NULL AND checked_in_at IS NOT NULL`).get() as Record<string, unknown>;
  ...
  return {
    ...
    removed: Number(removed.n), removedApproved: Number(removedApproved.n),
    checkedIn: Number(checkedInPaid.n) + Number(checkedInComplimentary.n),
    checkedInPaid: Number(checkedInPaid.n), checkedInComplimentary: Number(checkedInComplimentary.n),
